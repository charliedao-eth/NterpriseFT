...
# All ETH Sales Over Time Within IQR Range
```{r echo=FALSE}
message('---All ETH Sales Over Time within IQR Range---')
...
for (nftName in keys(dataframeHash)) {
  message(paste0('\n', nftName, ':', '\n'))
  # iqr filter attempts to remove extremely high values that don't seem possible
  plotDf <- values(dataframeHash, keys=nftName)[[1]] %>%
    dplyr::filter(
      PRICE >= quantile(PRICE, 0.25, na.rm=TRUE),
      PRICE <= quantile(PRICE, 0.75, na.rm=TRUE)
    )
  ...
  createNftTimeSeriesPlot(
    ...
    outcomeCols='PRICE',
    nftName=nftName,
    title='All ETH Sales Over Time Within IQR Range',
    imputeData=FALSE,
    geomLine=FALSE,
    logScale=FALSE,
    savePlotData=TRUE
